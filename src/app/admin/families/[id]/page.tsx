...
export default async function FamilyDetail({ params }: { params: Promise<{ id: string }> }) {
    ...
    // Metrics Rollup
    const allTransactions = household.members.flatMap((m: any) =>
        m.person.transactions.map((t: any) => ({ ...t, personName: (m.person.firstName || '') + ' ' + (m.person.lastName || '') }))
    );
    ...
    return (
        <div className="container" style={{ maxWidth: '1000px' }}>
            <div style={{ marginBottom: '2rem' }}>
                <Link href="/admin/families" style={{ display: 'inline-flex', alignItems: 'center', color: '#6b7280', textDecoration: 'none', marginBottom: '1rem' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    {isSuperAdmin && <DeleteFamilyButton householdId={household.id} householdName={household.householdName} />}
                </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '2rem' }}>
                ...
                <div className="card" style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: '1.5rem', fontWeight: 700 }}>{household.members.length}</div>
                </div>
                <div className="card" style={{ textAlign: 'center' }}>
                    ...
                    <div className="card" style={{ padding: 0 }}>
                        {household.members.map((m: any) => (
                            <div key={m.id} style={{ padding: '1rem', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center' }}>
                                ...
                                <div>
                                    <Link href={`/admin/people/${m.person.id}`} style={{ fontWeight: 500, color: '#1f2937', textDecoration: 'none', display: 'block' }}>
