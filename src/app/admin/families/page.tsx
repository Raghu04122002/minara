...
export default async function FamiliesList() {
    ...
    type FamilyType = typeof households[0];
    ...
    const familiesWithStats: FamilyWithStats[] = households.map((f: FamilyType) => {
        // Rollup transactions from all people in household
        const allTransactions: TransactionType[] = f.members.flatMap((m: any) => m.person.transactions);
        ...
        <div className="container">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                    {isSuperAdmin && <Link href="/admin/families/new" className="btn" style={{ background: '#22c55e', color: 'white' }}>+ Create Household</Link>}
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                    <thead>
                        <tr style={{ background: '#f9fafb', textAlign: 'left' }}>
                            <th style={{ padding: '0.75rem 1.5rem', borderBottom: '1px solid #e5e7eb' }}>Household Name</th>
                    ...
                    <tbody>
                            ...
                            return (
                                <tr key={f.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                                    <td style={{ padding: '0.75rem 1.5rem' }}>
                                        <Link href={`/admin/families/${f.id}`} style={{ color: '#2563eb', fontWeight: 500, textDecoration: 'none' }}>
                                            {f.householdName}
                                        </Link>
                                    </td>
                                    <td style={{ padding: '0.75rem 1.5rem' }}>
                        ...
                        {familiesWithStats.length === 0 && (
                            <tr><td colSpan={5} style={{ padding: '2rem', textAlign: 'center', color: '#6b7280' }}>No households formed yet. Run householding logic!</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
